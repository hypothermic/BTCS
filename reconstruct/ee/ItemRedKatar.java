...
public class ItemRedKatar extends ItemRedTool
{
  ...
  private static Block[] blocksEffectiveAgainst = { Block.WOOD, Block.BOOKSHELF, Block.LOG, Block.CHEST, Block.DIRT, Block.GRASS, Block.LEAVES, Block.WEB, Block.WOOL };
  ...
  public float getStrVsBlock(ItemStack var1, Block var2, int var3)
  {
    ...
    if ((var2.id != Block.VINE.id) && (var2.id != Block.LEAVES.id) && (var2.id != Block.WEB.id))
    {
      ...
      float var4 = 18.0F + chargeLevel(var1) * 4;
      return var4;
    }
  ...
  public void doSwordBreak(ItemStack var1, World var2, EntityHuman var3)
  {
    if (chargeLevel(var1) > 0)
    {
      ...
      for (int var5 = 1; var5 <= chargeLevel(var1); var5++)
      {
      ...
      List var6 = var2.getEntities(var3, net.minecraft.server.AxisAlignedBB.b((float)var3.locX - (var5 / 1.5D + 2.0D), var3.locY - (var5 / 1.5D + 2.0D), (float)var3.locZ - (var5 / 1.5D + 2.0D), (float)var3.locX + var5 / 1.5D + 2.0D, var3.locY + var5 / 1.5D + 2.0D, (float)var3.locZ + var5 / 1.5D + 2.0D));
      
      for (int var7 = 0; var7 < var6.size(); var7++)
      {
        if (((var6.get(var7) instanceof EntityLiving)) && ((EEBase.getSwordMode(var3)) || ((var6.get(var7) instanceof net.minecraft.server.EntityMonster))))
        {
          Entity var8 = (Entity)var6.get(var7);
          var8.damageEntity(net.minecraft.server.DamageSource.playerAttack(var3), this.weaponDamage + chargeLevel(var1) * 2);
        }
      }
    }
  }
      ...
      for (int var9 = -(chargeLevel(var1) + 2); var9 <= chargeLevel(var1) + 2; var9++)
      {
        for (int var10 = -(chargeLevel(var1) + 2); var10 <= chargeLevel(var1) + 2; var10++)
        {
          for (int var11 = -(chargeLevel(var1) + 2); var11 <= chargeLevel(var1) + 2; var11++)
          {
            ...
            if ((EEMaps.isLeaf(var12)) || (var12 == Block.VINE.id) || (var12 == Block.WEB.id) || (var12 == BlockFlower.LONG_GRASS.id) || (var12 == BlockFlower.DEAD_BUSH.id))
            {
              ...
              if (getFuelRemaining(var1) > 0)
              {
                ...
                {
                  addToDroplist(var1, new ItemStack(Block.LEAVES.id, 1, var13 & 0x3));
                }
                else
                {
  ...
  public net.minecraft.server.EnumAnimation d(ItemStack var1)
  {
    return net.minecraft.server.EnumAnimation.d;
  }
