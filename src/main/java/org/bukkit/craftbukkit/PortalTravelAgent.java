...
public class PortalTravelAgent implements TravelAgent
{
  ...
  public Location findOrCreate(Location location)
  {
    ...
    {
      if ((this.canCreatePortal) && (createPortal(location)))
      {
  ...
  public Location findPortal(Location location) {
    ...
    if (location.getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END) { // BTCS: 'World' --> 'org.bukkit.World'
      ...
      for (int l = -2; l <= 2; l++) {
        for (int i1 = -2; i1 <= 2; i1++) {
          for (int j1 = -1; j1 < 3; j1++) {
            ...
            if (world.getTypeId(k1, l1, i2) != (flag ? net.minecraft.server.Block.OBSIDIAN.id : 0)) {
              return null;
            }
          }
        }
    ...
    for (int j1 = l - this.searchRadius; j1 <= l + this.searchRadius; j1++) {
      ...
      for (int k1 = i1 - this.searchRadius; k1 <= i1 + this.searchRadius; k1++) {
        ...
        for (int l1 = 127; l1 >= 0; l1--) {
          if (world.getTypeId(j1, l1, k1) == net.minecraft.server.Block.PORTAL.id) {
            while (world.getTypeId(j1, l1 - 1, k1) == net.minecraft.server.Block.PORTAL.id) {
              l1--;
            }
    ...
    if (d0 >= 0.0D) {
      ...
      double d1 = k + 0.5D;
      if (world.getTypeId(i - 1, j, k) == net.minecraft.server.Block.PORTAL.id) {
        d5 -= 0.5D;
      }
      ...
      if (world.getTypeId(i, j, k - 1) == net.minecraft.server.Block.PORTAL.id) {
        d1 -= 0.5D;
      }
  ...
  public boolean createPortal(Location location)
  {
      ...
      for (int l = -2; l <= 2; l++) {
        for (int i1 = -2; i1 <= 2; i1++) {
          for (int j1 = -1; j1 < 3; j1++) {
            ...
            world.setTypeId(k1, l1, i2, flag ? net.minecraft.server.Block.OBSIDIAN.id : 0);
          }
        }
      }
    ...
    for (int i2 = i - this.creationRadius; i2 <= i + this.creationRadius; i2++) {
      double d1 = i2 + 0.5D - location.getX();
      
      for (int j2 = k - this.creationRadius; j2 <= k + this.creationRadius; j2++) {
    ...
    if (d0 < 0.0D) {
      for (int i2 = i - this.creationRadius; i2 <= i + this.creationRadius; i2++) { // BTCS: 'i2 =' --> 'int i2 ='
        double d1 = i2 + 0.5D - location.getX();
    ...
    if (d0 < 0.0D) {
      ...
      for (int l2 = -1; l2 <= 1; l2++) { // BTCS: 'l2 =' --> 'int l2 ='
        for (int k2 = 1; k2 < 3; k2++) {
          for (int j3 = -1; j3 < 3; j3++) {
            int i3 = i5 + (k2 - 1) * k5 + l2 * l5;
            int l3 = j5 + j3;
            int k3 = j2 + (k2 - 1) * l5 - l2 * k5;
            boolean flag = j3 < 0;
            world.setTypeId(i3, l3, k3, flag ? net.minecraft.server.Block.OBSIDIAN.id : 0);
          }
        }
      }
    }
    
    for (int l2 = 0; l2 < 4; l2++) { // BTCS: 'l2 =' --> 'int l2 ='
      ...
      for (int k2 = 0; k2 < 4; k2++) {
        for (int j3 = -1; j3 < 4; j3++) {
          ...
          world.setTypeId(i3, l3, k3, flag ? net.minecraft.server.Block.OBSIDIAN.id : net.minecraft.server.Block.PORTAL.id);
        }
      }
      
      world.suppressPhysics = false;
      
      for (int k2 = 0; k2 < 4; k2++) { // BTCS: 'k2 =' --> 'int k2 ='
        for (int j3 = -1; j3 < 4; j3++) {
          int i3 = i5 + (k2 - 1) * k5;
          int l3 = j5 + j3;
          int k3 = j2 + (k2 - 1) * l5;
