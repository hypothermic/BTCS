  ...
  implements BlockSapling.TreeGenerator
{
  ...
  public boolean generate(BlockChangeDelegate world, Random random, int i, int j, int k)
  {
    ...
    for (l = random.nextInt(4) + 5; (world.getTypeId(i, j - 1, k) != 0) && (Block.byId[world.getTypeId(i, j - 1, k)].material == Material.WATER); j--) {}
    ...
    if ((j >= 1) && (j + l + 1 <= 128))
    {
    	...
    	int i1; // BTCS: moved outside for loop
      ...
      if (((i1 == Block.GRASS.id) || (i1 == Block.DIRT.id)) && (j < 128 - l - 1)) {
        ...
        int j2; //  BTCS: moved outside for loop
