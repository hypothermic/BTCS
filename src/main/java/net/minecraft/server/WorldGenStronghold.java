  ...
  protected boolean a(int paramInt1, int paramInt2)
  {
    ...
    if (!this.f)
    {
      ...
      for (int i = 0; i < this.g.length; i++) {
        ...
        // BTCS comment: replaced '???' with Object 'x' in the next couple of lines, it should hopefully work.
        Object x = this.d.getWorldChunkManager().a((j << 4) + 8, (k << 4) + 8, 112, localArrayList, (Random)localObject1);
        if (x != null) {
          j = ((ChunkPosition)x).x >> 4;
          k = ((ChunkPosition)x).z >> 4;
        }
        else {
