...
public class TileEntityFurnace extends TileEntity implements IInventory, ISidedInventory { // BTCS: isidedinventory
    ...
    public static int fuelTime(ItemStack itemstack) {
        if (itemstack == null) {
            ...
            int var1 = itemstack.getItem().id;
            if (itemstack.getItem() instanceof ItemBlock && Block.byId[var1].material == Material.WOOD) return 300;
            if (var1 == Item.STICK.id) return 100;
            if (var1 == Item.COAL.id) return 1600;
            if (var1 == Item.LAVA_BUCKET.id) return 20000;
            if (var1 == Block.SAPLING.id) return 100;
            if (var1 == Item.BLAZE_ROD.id) return 2400;
            int ret = ForgeHooks.getItemBurnTime(itemstack);
            if (ret > 0) {
                return ret;
            }
            return FMLCommonHandler.instance().fuelLookup(var1, itemstack.getDamage());
            // BTCS end
        }
    }
    ...
    // BTCS start: equivalents of f() and g()
