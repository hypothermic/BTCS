...
public class EntityGhast extends EntityFlying implements IMonster
{
  ...
  protected void dropDeathLoot(boolean flag, int i)
  {
    ...
    if (j > 0) {
      loot.add(new org.bukkit.craftbukkit.inventory.CraftItemStack(Item.GHAST_TEAR.id, j));
    }
    
    j = this.random.nextInt(3) + this.random.nextInt(1 + i);
    
    if (j > 0) {
      loot.add(new org.bukkit.craftbukkit.inventory.CraftItemStack(Item.SULPHUR.id, j));
    }
