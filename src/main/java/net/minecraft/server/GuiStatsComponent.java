...
import forge.DimensionManager; // BTCS

// BTCS: imported in v1.07
...
public class GuiStatsComponent extends JComponent {
  ...
  private void a() {
    ...
    // BTCS start
    if (d.length < DimensionManager.getIDs().length + 3) {
        d = new String[DimensionManager.getIDs().length + 3];
    }
    ...
    this.d[2] = ("Avg tick: " + a.format(a(this.e.f) * 1.0E-6D) + " ms");
    this.d[3] = ("Avg sent: " + (int)a(this.e.u) + ", Avg size: " + (int)a(this.e.v));
    this.d[4] = ("Avg rec: " + (int)a(this.e.w) + ", Avg size: " + (int)a(this.e.x));
    // BTCS TODO: figure out how to implement forge patch in this messy code. Low priority.
    if (this.e.worlds != null) {
      for (int i = 0; i < this.e.worlds.size(); i++) { // BTCS: 'tmp408_407' --> 'i'
        this.d[(5 + i)] = ("Lvl " + i + " tick: " + a.format(a(this.e.g[i]) * 1.0E-6D) + " ms");
        if ((this.e.worlds.get(i) != null) && (this.e.worlds.get(i).chunkProviderServer != null)) {
          int tmp408_407 = (5 + i); String[] tmp408_402 = this.d;tmp408_402[tmp408_407] = (tmp408_402[tmp408_407] + ", " + this.e.worlds.get(i).chunkProviderServer.d());
        }
      }
    }
  ...
  private double a(long[] paramArrayOfLong) {
    long l = 0L;
    if (paramArrayOfLong == null) return 0; // BTCS
  ...
  public void paint(Graphics paramGraphics) {
    ...
    for (int i = 0; i < this.d.length; i++) { // BTCS: added 'int ' decl
      String str = this.d[i];
