...
public class EntityEnderDragon extends EntityComplex
{
  ...
  public double[] a(int i, float f) {
    ...
    double d1; // BTCS: moved decl from ln 71
    ...
    if (this.health <= 0) {
      ...
      if (this.world.isStatic) {
        if (this.aN > 0) {
          ...
          double d3; // BTCS: moved decl from ln 146 within for loop
          for (d3 = this.aR - this.yaw; d3 < -180.0D; d3 += 360.0D) {}
        ...
        double d9; // BTCS: moved decl from ln 210
        ...
        Vec3D vec3d1 = Vec3D.create(MathHelper.sin(this.yaw * 3.1415927F / 180.0F), this.motY, -MathHelper.cos(this.yaw * 3.1415927F / 180.0F)).b();
      ...
      this.m.length = 3.0F;
      this.m.width = 4.0F;
      float d05 = (float)(a(5, 1.0F)[1] - a(10, 1.0F)[1]) * 10.0F / 180.0F * 3.1415927F;
      ...
      float f3 = MathHelper.sin(this.yaw * 3.1415927F / 180.0F - this.aY * 0.01F);
      ...
      for (int j = 0; j < 3; j++) {
        ...
        float f14 = this.yaw * 3.1415927F / 180.0F + a(adouble2[0] - adouble[0]) * 3.1415927F / 180.0F * 1.0F;
  ...
  private void a(int i, int j) {
    ...
    if (!event.isCancelled()) {
      ...
      for (BlockState state : event.getBlocks()) {
        Packet53BlockChange packet = new Packet53BlockChange(state.getX(), state.getY(), state.getZ(), this.world); // BTCS: added decl Packet53BlockChange
        for (Iterator it = this.world.players.iterator(); it.hasNext();) { // BTCS added decl Iterator
