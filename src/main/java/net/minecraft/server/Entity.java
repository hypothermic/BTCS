...
public abstract class Entity implements net.minecraft.src.Entity
{
  ...
  public static enum EntitySize
  {
    ...
    private EntitySize() {}
    public int getXZCoord(double loc) { double diff = loc - (NumberConversions.floor(loc) + 0.5D);
    
      // BTCS start
      /*switch (Entity.1.$SwitchMap$net$minecraft$server$Entity$EntitySize[ordinal()]) {*/
      switch (Entity.EntitySize.values()[ordinal()].ordinal()) { // not sure if this is correct
      // BTCS end
      case 1: 
  ...
  public Entity(World world) { this.id = (entityCount++);
    this.be = 1.0D;
    this.bf = false;
  ...
  public void aA()
  {
    ...
    if (h_()) {
      if ((!this.bV) && (!this.justCreated)) {
        ...
        for (int i = 0; i < 1.0F + this.width * 20.0F; i++) { // BTCS: added 'int '
          float f3 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
          float f2 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
  ...
  public void move(double d0, double d1, double d2) {
    if (this.bQ) {
      ...
      if (flag)
      {
        ...
        for (; // BTCS: removed 'double d8' 
        		(d2 != 0.0D) && (this.world.getCubes(this, this.boundingBox.c(0.0D, -1.0D, d2)).size() == 0); d7 = d2) {
          if ((d2 < d8) && (d2 >= -d8)) {
            d2 = 0.0D;
          } else if (d2 > 0.0D) {
            d2 -= d8;
      ...
      for (int j = 0; j < list.size(); j++) { // BTCS: added 'int '
      ...
      if ((this.bP > 0.0F) && (flag1) && ((flag) || (this.bO < 0.05F)) && ((d5 != d0) || (d7 != d2))) {
        ...
        for (int k = 0; k < list.size(); k++) { // BTCS: added 'int '
      ...
      if ((g_()) && (!flag) && (this.vehicle == null)) {
        ...
        int i1 = MathHelper.floor(this.locY - 0.20000000298023224D - this.height);
  ...
  public void a(EntityWeatherLighting entityweatherlighting)
  {
    ...
    if ((thisBukkitEntity instanceof org.bukkit.entity.Painting)) {
      PaintingBreakByEntityEvent event = new PaintingBreakByEntityEvent((org.bukkit.entity.Painting)thisBukkitEntity, stormBukkitEntity);
      pluginManager.callEvent(event);
