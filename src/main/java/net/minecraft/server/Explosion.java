...
public class Explosion
{
  ...
  public void a()
  {
    ...
    this.size *= 2.0F;
    int i = MathHelper.floor(this.posX - this.size - 1.0D); // BTCS: added 'int '
    ...
    for (int k2 = 0; k2 < list.size(); k2++) {
      ...
      if (d7 <= 1.0D)
      {
        ...
        {
          if (this.source == null) {
            EntityDamageByBlockEvent event = new EntityDamageByBlockEvent(null, damagee, org.bukkit.event.entity.EntityDamageEvent.DamageCause.BLOCK_EXPLOSION, damageDone); // BTCS: added 'org.bukkit.event.entity.'
  ...
  public void a(boolean flag) {
    ...
    if (this.a) {
      for (int i = arraylist.size() - 1; i >= 0; i--) { // BTCS: added 'int '
        ChunkPosition chunkposition = (ChunkPosition)arraylist.get(i);
        int j = chunkposition.x;
        int k = chunkposition.y;
        int l = chunkposition.z;
