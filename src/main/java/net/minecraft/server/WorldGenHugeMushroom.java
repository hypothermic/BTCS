...
public class WorldGenHugeMushroom extends WorldGenerator implements BlockSapling.TreeGenerator
{
  ...
  public boolean grow(BlockChangeDelegate world, Random random, int i, int j, int k, StructureGrowEvent event, ItemStack itemstack, CraftWorld bukkitWorld)
  {
    ...
    if ((j >= 1) && (j + i1 + 1 < 256))
    {
    	...
    	int j1; // BTCS: moved outside of for loop
          ...
          for (int k2 = k - l1; k2 <= k + l1; k2++) {
            ...
            if (((l2 != 0) || (j >= j + i1 - 1)) && ((bl == null) || (bl.canBeReplacedByLeaves(world, i2, k1, k2))))
            {
              if (event == null) {
                ...
                state.setData(new org.bukkit.material.MaterialData(Block.BIG_MUSHROOM_1.id + l, (byte)l2));
      ...
      for (int k1 = 0; k1 < i1; k1++) { int l1;  // BTCS: added decl 'int '
        ...
        if ((bl == null) || (bl.canBeReplacedByLeaves(world, i, j + k1, k)))
        {
          if (event == null) {
            ...
            state.setTypeId(Block.BIG_MUSHROOM_1.id + l);
            state.setData(new org.bukkit.material.MaterialData(Block.BIG_MUSHROOM_1.id + l, (byte)10));
            event.getBlocks().add(state);
          }
        }
      }
